...
const UserStats: React.FC = () => {
	...
	// Nationality options (from the documentation link provided)
	const nationalities = [
	...
	useEffect(() => {
		const loadUsers = async () => {
			// Only fetch if we have valid input values
	...
	}, [userCount, nationality]); // This will trigger a new fetch whenever these values change
	...
	const genderChartData = Object.entries(stats.genderStats).map(([label, value]) => ({
		label,
		value
	}));

	const ageChartData = Object.entries(stats.ageRangeStats).map(([label, value]) => ({
		label,
		value
	}));

	const statesChartData = Object.entries(stats.topStatesStats).map(([label, value]) => ({
		label,
		value
	}));
	...
	return (
		<div className="flex flex-col gap-8">
			<h1 className="text-4xl font-bold text-center mb-4">Random User Statistics</h1>

			{/* Input Form with DaisyUI styling */}
					...
					<div className="mt-4">
						<div className="flex flex-col md:flex-row gap-4 justify-center items-end">
							<div className="form-control w-full max-w-xs">
								...
								/>
								<label className="label">
									<span className="label-text-alt">Data updates automatically as you type</span>
								</label>
							</div>
			...
			{loading ? (
				<div className="flex justify-center items-center h-64">
					<div className="card bg-base-100 shadow-xl p-12">
						<span className="mt-4 loading loading-spinner loading-lg text-primary"></span>
				...
				<div className="flex flex-col gap-8">
					...
					<div className="stats shadow w-full bg-primary text-primary-content">
						<div className="stat">
							...
							<div className="stat-value text-4xl">{users.length}</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
						...
						<div className="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow duration-300">
							<div className="card-body">
								...
								{Object.entries(stats.genderStats).map(([gender, percentage]) => (
									<div key={gender} className="flex justify-between items-center mb-2">
										<span className="capitalize">{gender}</span>
										<div className="flex items-center">
											<div className="badge badge-primary mr-2">{percentage.toFixed(1)}%</div>
											<progress
								...
								<div className="flex justify-center mt-6">
									<PieChart data={genderChartData} title="Gender Distribution" />
								</div>
							</div>
						</div>
						...
						<div className="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow duration-300">
							<div className="card-body">
								...
								<div className="divider"></div>
								{Object.entries(stats.ageRangeStats).map(([ageRange, percentage]) => (
									<div key={ageRange} className="flex justify-between items-center mb-2">
										<span>{ageRange}</span>
										<div className="flex items-center">
											<div className="badge badge-secondary mr-2">{percentage.toFixed(1)}%</div>
											<progress
							...
							<div className="card-body">
								...
								<div className="overflow-x-auto">
									<table className="table table-zebra w-full">
										<thead>
											<tr>
												<th className="bg-accent text-accent-content">Length</th>
												<th className="bg-accent text-accent-content">Count</th>
											</tr>
										</thead>
										<tbody>
											{Object.entries(stats.lastNameLengthStats).sort((a, b) =>
						...
						<div className="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow duration-300">
							<div className="card-body">
								...
								<div className="divider"></div>
								{Object.entries(stats.topStatesStats).map(([state, percentage]) => (
									<div key={state} className="flex justify-between items-center mb-2">
										<span>{state}</span>
										<div className="flex items-center">
											<div className="badge badge-info mr-2">{percentage.toFixed(1)}%</div>
											<progress
								...
								<div className="flex justify-center mt-6">
									<PieChart data={statesChartData} title="Top States Distribution" />
								</div>
							</div>
						</div>
					</div>
